...
class Userlog(db.Model):
    ...
    uer_id = db.Column(db.Integer, db.ForeignKey('user.id'))  # 绑定外键两步走，所属会员
...
class Admin(db.Model):
    __tablename__ = "admin"
    id = db.Column(db.Integer, primary_key=True)  # 管理员id
    ...
    addtime = db.Column(
        ...
        index=True,
        default=datetime.now)  # 加入时间
    adminlogs = db.relationship('Adminlog', backref='admin')  # 管理员登陆日志外键关系绑定
...
class Adminlog(db.Model):
    ...
    admin_id = db.Column(
        db.Integer,
        db.ForeignKey('admin.id'))  # 绑定外键两步走，所属管理员
    ip = db.Column(db.String(100))  # 登陆ip
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)
